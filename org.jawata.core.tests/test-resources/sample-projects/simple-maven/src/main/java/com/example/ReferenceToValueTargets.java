...
public class ReferenceToValueTargets {
    ...
    /**
     * Two 1-arg setters of ONE name, on different fields — the second axis of the same defect.
     *
     * <p>Carrying a setter between steps as a bare name cannot tell these apart: the row would
     * emit the name twice and resolve both steps to whichever overload comes first, so one
     * field is done twice and the other never. Both are plain setting methods, so the row must
     * remove both.</p>
     */
