...
| `tools/smell/ReDerivedJobDetector` (new) | S9: nominates re-derived jobs on compiler facts — same signature shape, overlapping foreign symbols, no IMPOSED signature, no reference between them (**amended at C8**: this said "no common supertype", which was measured wrong while building the detector — the ~34 parse helpers this smell exists to name all extend `AbstractApplyingRefactoringTool`, so a common-supertype clause silences exactly the population. What disqualifies a member is a signature IMPOSED from above — an override or an interface method — because that is a shape its author did not choose); token clones stay `find_duplicate_code`'s | `find_duplicate_code` = exact normalised tokens; the smell family in `tools/smell/` with `CureCatalog` routing |
...
| M8 | **Retrieval wired** — map at prompt; area on first Read; `dupgate.rs` (**Observe — declared deviation, see §5 note 5**) + `duplicate_check` (draft parsed standalone, NOT as a JDT working copy — deviation two); stop rule for jobs; `ReDerivedJobDetector` + `CureCatalog` row; architect detect step reads the population | M7 | Rust: judge fixtures; engine: a draft re-deriving a fixture job is refused naming it, declared proceeds; the detector names the fixture populations and the token detector names none | disable the disposition token → red |
...
5. **`dupgate.rs` is a copy of `recallgate.rs`'s shape, not a parameterisation** — two gates with one declaration lattice each stay readable; a generic gate with two token vocabularies is the abstraction nobody asked for. **AMENDED AT C8: it ships in OBSERVE, not Block.** The ruling said Block; the measurement that overrode it is the store's own — nomination applies no score threshold (Stage 0: 3 of 10 genuine pairs score at or below the noise floor), so rank one always comes back and a gate denying on it would deny every Java write. The kill switch ships as designed. A second deviation sits under it: `DraftSource` parses the draft standalone rather than as a JDT working copy, so the confirming structural signal has no bindings to run on — that is work not done rather than a wall, and the mode does not rest on it.
